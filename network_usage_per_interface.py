...
while True:
    ...
    # get the network I/O stats again per interface (dictionary)
    io_2 = psutil.net_io_counters(pernic=True)
    
    # check if all interfaces are still online
    # debug issue when interface disapears
    diff_key = set(io) - set(io_2)
    if diff_key:
        print("removed interface", diff_key)
        for each in diff_key:
                print(each)
                del io[each]
        # del io[diff_key]
        # dict(io_2.items() - io.items())
